...
namespace _09_DesignDatabaseConnection 
{
    class Program 
    {
        ...
        {
            var sqlConnection = new SqlConnection("Server=LTIN1234\\SQLExpress;Database=myDataBase;User Id=myUsername;Password=myPassword;", TimeSpan.FromSeconds(30));
            sqlConnection.Open();
            sqlConnection.Close();

            var oracleConnection = new OracleConnection("Data Source=LocalHost\\MyOracleDB;User Id=myUsername;Password=myPassword;", TimeSpan.FromSeconds(30));
            oracleConnection.Open();
            oracleConnection.Close();

            try {
                var invalidConnection = new SqlConnection("", TimeSpan.FromSeconds(10));
